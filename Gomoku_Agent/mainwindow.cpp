...
// 请务必确认这两个文件存在于构建后的可执行文件同级目录，或者使用绝对路径
#define ASTAR_AI_PATH "/Users/chris/Desktop/Gomoku/ai_stdin.py"
#define RL_AI_PATH    "/Users/chris/Desktop/Gomoku/RL.py"
...
void MainWindow::createUi() {
    ...
    // 1. 实例化两个 AI 进程
    PythonAiProcess *astarClient = new PythonAiProcess(ASTAR_AI_PATH, this);
    PythonAiProcess *rlClient    = new PythonAiProcess(RL_AI_PATH, this);

    // 2. 创建控制器
    m_controller = new GameController(m_model, astarClient, rlClient, this);
    // 修改第 40 行
    connect(m_controller, &GameController::boardUpdated,
            m_board, qOverload<>(&QWidget::update));

    // 3. 启动 A* (黑) vs RL (白)
    m_controller->startNewGame(PlayerKind::RLAI, PlayerKind::AStarAI);

    // 注意：AI vs AI 模式不需要连接 cellClicked 信号
...
void MainWindow::onNewGame() {
    ...
    if (ok) {
        m_controller->startNewGame(PlayerKind::AStarAI, PlayerKind::RLAI);
    }
}
